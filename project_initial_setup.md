...
| Template | URP 2D template (`Assets/Settings/Renderer2D.asset`) | URP **3D Forward** + 3D orthogonal camera | Renderer must be swapped, not configured |
| Renderer wiring | `UniversalRP.asset` → renderer list = `Renderer2D` | `UniversalRendererData` (3D) | Must add 3D renderer + remove 2D |
| Scene | Single `SampleScene.unity`, flat top-down camera, hosts `Line98Game` | `Boot` / `Menu` / `Game`, 3D orthogonal camera (pitch 58°, ortho size ~7.5) | Scenes don't exist |
